...
import { financeFields } from '@infra/data/static/forms/finance/finance';
...
export class NovaFinancaComponent implements OnInit {
      ...
      this.financeFormFields.fields.reduce(
        (formFields, field) => {
          formFields[field.name] = [
            field.value || '',
            this._formValidateService.bindValidations(field.validations || []),
          ];
          return formFields;
        },
  ...
  onSubmit(): void {
    if (this.financeForm.valid) {
      ...
      console.log('Formulário inválido', this.financeForm.errors);
      Object.keys(this.financeForm.controls).forEach(key => {
        const controlErrors = this.financeForm.get(key)?.errors;
        if (controlErrors) {
